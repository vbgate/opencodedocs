---
title: "opencode-notify 工作原理：四种通知类型与智能过滤机制详解 | 教程"
sidebarTitle: "为什么不会被打扰"
subtitle: "工作原理：通知、过滤与触发机制"
description: "深入了解 opencode-notify 的工作原理，掌握四种通知类型（任务完成、错误、权限请求、问题询问）、三层智能过滤机制（父会话检查、静音时段、终端焦点检测）以及完整的通知触发流程，学会通过配置文件自定义通知行为，理解 macOS 与 Windows/Linux 的平台差异，提升工作效率与专注度。"
tags:
  - "原理"
  - "通知机制"
  - "智能过滤"
...
sequenceDiagram
    ...
    AI->>OC: 会话完成 (session.idle)
    OC->>Plugin: 触发事件
    Plugin->>Plugin: 检查是否父会话
    alt 子会话
        Plugin->>Plugin: 🚫 不通知
    else 父会话
        Plugin->>Plugin: 检查静音时段
        alt 在静音时段
            Plugin->>Plugin: 🚫 不通知
        else 非静音时段
            Plugin->>Plugin: 检查终端焦点
            alt 终端已聚焦
                Plugin->>Plugin: 🚫 不通知
            else 终端未聚焦
                Plugin->>OS: 发送原生通知

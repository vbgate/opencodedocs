---
title: "快速开始: 安装与配置 | opencode-supermemory"
sidebarTitle: "给 Agent 装个大脑"
subtitle: "快速开始: 安装与配置"
description: "学习如何安装 opencode-supermemory 插件，配置 API Key，并解决与 Oh My OpenCode 的冲突，让 Agent 拥有持久化记忆。"
tags:
  - "安装"
  - "配置"
  - "入门"
...
在本课中，你将学会：
1.  将 **opencode-supermemory** 插件安装到你的 OpenCode 环境中。
...
2.  **获取了 API Key**：
    - 访问 [Supermemory Console](https://console.supermemory.ai)
    - 注册/登录账号
    - 创建一个新的 API Key（以 `sm_` 开头）

::: info 什么是 Supermemory？
...
**你应该看到**：
终端输出 `✓ Setup complete!`，表示插件已成功注册到 `~/.config/opencode/opencode.jsonc`。
...
如果你安装了 [Oh My OpenCode](https://github.com/code-yeongyu/oh-my-opencode)，它自带的上下文管理功能可能会与 Supermemory 冲突。

**检查方法**：
安装脚本通常会自动检测并提示你禁用冲突的钩子。如果没有，请手动检查 `~/.config/opencode/oh-my-opencode.json`：
...
**你应该看到**：
在工具列表（Tools）中，出现了 `supermemory` 工具。
...
- [ ] 运行 `cat ~/.config/opencode/opencode.jsonc`，能看到 `"opencode-supermemory"` 在 `plugin` 列表中。
...
> 下一课我们学习 **[项目初始化：建立第一印象](../initialization/index.md)**。
>
> 你会学到：
...
| 配置文件加载 | [`src/config.ts`](https://github.com/supermemoryai/opencode-supermemory/blob/main/src/config.ts) | - |

---
title: "后台任务: 并行执行多个代理 | oh-my-opencode"
sidebarTitle: "让多个 AI 同时干活"
subtitle: "后台任务: 并行执行多个代理 | oh-my-opencode"
description: "学习 oh-my-opencode 后台任务系统的并行执行能力。掌握三级并发控制、任务生命周期管理、使用 delegate_task 和 background_output 工具。"
tags:
  - "background-tasks"
  - "parallel-execution"
  - "concurrency"
...
3. 了解 Sisyphus 编排器的基础用法（见 [Sisyphus 教程](../sisyphus-orchestrator/)）
...
**为什么**
这是演示后台任务最经典的使用场景。3 个任务可以同时进行，大幅节省时间。
...
**为什么**
并发控制是避免成本失控的关键。如果你没有设置限制，同时启动 10 个 Opus 4.5 任务，可能会瞬间消耗大量 API 配额。
...
> 下一课我们学习 **[LSP 与 AST-Grep：代码重构利器](../lsp-ast-tools/)**。
>
> 你会学到：
...
| background_output 工具 | [`src/tools/background-task/tools.ts`](https://github.com/code-yeongyu/oh-my-opencode/blob/main/src/tools/background-task/tools.ts) | 320-384 |
| background_cancel 工具 | [`src/tools/background-task/tools.ts`](https://github.com/code-yeongyu/oh-my-opencode/blob/main/src/tools/background-task/tools.ts) | 386-514 |

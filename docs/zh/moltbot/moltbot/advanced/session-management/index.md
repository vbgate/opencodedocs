...
```json5
{
  ...
  bindings: [
    ...
    // 其他 WhatsApp 消息路由到 home Agent（默认）
    { agentId: "home", match: { channel: "whatsapp" } }
  ],
  tools: {
    agentToAgent: {
...
**你应该看到**：Gateway 启动日志，显示已加载的 Agent 和绑定规则。
...
```
请用子 Agent 分析这个 PDF 文档并提取关键信息：/path/to/file.pdf
```
...
| 错误 | 原因 | 解决方法 |
|--- | --- | ---|
| **Agent 间调用被禁止** | `tools.agentToAgent.enabled` 未启用或 `allow` 列表不包含目标 Agent | 检查配置，确保已启用并添加允许列表 |
| **子 Agent 创建失败** | 跨 Agent 调用时，目标 Agent 不在允许列表中 | 检查 `tools.agentToAgent.allow` 配置 |
| **会话键冲突** | 绑定规则顺序错误，更具体的规则被覆盖 | 将 peer 规则放在渠道规则之前 |
| **压缩失败** | 上下文过小或模型不支持压缩 | 检查模型上下文窗口和压缩配置 |

### 配置建议

- **从简单开始**：先用单 Agent 测试，确认功能正常后再添加多 Agent
- **使用标签**：给子 Agent 任务添加清晰的 `label`，方便追踪
- **监控 token 使用**：定期检查 `/status`，了解上下文使用情况
- **备份配置**：修改 `clawdbot.json` 前先备份

## 本课小结

本课介绍了 Clawdbot 的会话管理系统：

- **会话模型**：会话键、存储位置、生命周期
- **会话隔离**：多 Agent 模式、DM 范围控制、身份链接
- **子 Agent 协作**：`sessions_spawn` 工具、权限控制、清理策略
- **上下文压缩**：自动/手动压缩、压缩 vs 剪枝

通过合理配置会话管理，你可以让 Clawdbot 同时服务多个用户和场景，每个都有自己的隔离上下文和历史。

## 下一课预告

> 下一课我们学习 **[浏览器自动化工具](../tools-browser/)**。
>
> 你会学到：
> - 如何配置和使用浏览器工具
> - 浏览器自动化的安全限制
...
| 功能 | 文件路径 | 行号 |
|--- | --- | ---|
| 会话键解析 | [`src/routing/session-key.ts`](https://github.com/moltbot/moltbot/blob/main/src/routing/session-key.ts) | 1-100 |
| 会话配置 Schema | [`src/config/zod-schema.session.ts`](https://github.com/moltbot/moltbot/blob/main/src/config/zod-schema.session.ts) | 11-83 |
| 子 Agent 生成工具 | [`src/agents/tools/sessions-spawn-tool.ts`](https://github.com/moltbot/moltbot/blob/main/src/agents/tools/sessions-spawn-tool.ts) | 28-269 |
| 会话工具辅助函数 | [`src/agents/tools/sessions-helpers.ts`](https://github.com/moltbot/moltbot/blob/main/src/agents/tools/sessions-helpers.ts) | 1-328 |
| 会话管理文档 | [`docs/concepts/session.md`](https://github.com/moltbot/moltbot/blob/main/docs/concepts/session.md) | 1-151 |
| 多 Agent 路由文档 | [`docs/concepts/multi-agent.md`](https://github.com/moltbot/moltbot/blob/main/docs/concepts/multi-agent.md) | 1-355 |
| 上下文压缩文档 | [`docs/concepts/compaction.md`](https://github.com/moltbot/moltbot/blob/main/docs/concepts/compaction.md) | 1-50 |

**关键常量**：
- `SESSION_ID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i`：会话 ID 正则表达式（`sessions-helpers.ts:96`）

**关键类型**：
- `SessionKind = "main" | "group" | "cron" | "hook" | "node" | "other"`：会话类型枚举（`sessions-helpers.ts:11`）
- `SessionListRow`：会话列表行结构（`sessions-helpers.ts:19-41`）

**关键函数**：
- `createSessionsSpawnTool()`：创建子 Agent 生成工具（`sessions-spawn-tool.ts:60-269`）
- `resolveSessionReference()`：解析会话引用（`sessions-helpers.ts:215-253`）
- `classifySessionKind()`：分类会话类型（`sessions-helpers.ts:255-271`）

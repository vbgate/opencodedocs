---
title: "URL 分享: 无后端协作 | Plannotator"
sidebarTitle: "分享给团队看"
subtitle: "URL 分享: 无后端协作"
description: "学习 Plannotator 的 URL 分享功能。通过 deflate 压缩和 Base64 编码实现无后端协作，配置环境变量并解决常见分享问题。"
tags:
  - "URL 分享"
  - "团队协作"
  - "无后端"
...
- ✅ 区分分享模式（只读）和本地模式（可编辑）
- ✅ 配置 `PLANNOTATOR_SHARE` 环境变量控制分享功能
- ✅ 处理 URL 长度限制和分享失败的情况
...
**URL 分享**是 Plannotator 提供的一种无后端协作方式，通过将计划和注释压缩到 URL hash 中，实现无需服务器的分享功能。
...
**步骤 2：Deflate-raw 压缩**
- 使用原生 `CompressionStream('deflate-raw')` API
- 压缩率典型值为 60-80%（取决于文本重复度，非源码定义）
- 源码位置：`packages/ui/utils/sharing.ts:34`
...
- **基础域名**：`share.plannotator.ai`（独立分享页面）
- **Hash 分隔符**：`#`（不会发送到服务器，完全由前端解析）
- **压缩数据**：Base64url 编码的压缩 JSON

## 🎒 开始前的准备

**前置条件**：
- ✅ 已完成 [计划评审基础](../../platforms/plan-review-basics/)，了解如何添加注释
- ✅ 已完成 [计划注释教程](../../platforms/plan-review-annotations/)，了解注释类型
- ✅ 浏览器支持 `CompressionStream` API（现代浏览器均支持）
...
如果输出 `CompressionStream supported`，说明浏览器支持。现代浏览器（Chrome 80+、Firefox 113+、Safari 16.4+）均支持。
...
**操作**：
1. 在 Claude Code 或 OpenCode 中触发计划评审
2. 查看计划内容，选中需要修改的文本
3. 添加注释（删除、替换、评论等）
4. （可选）上传图片附件
...
**为什么**
Export Modal 提供了分享 URL 的生成入口。

**操作**：
1. 点击右上角的 **Export** 按钮
2. 等待 Export Modal 打开
...
**为什么**
复制 URL 后可以粘贴到任何通讯工具（Slack、Email、微信等）。
...
**为什么**
协作者通过打开 URL 可以查看计划和注释。

**操作**：
1. 将 URL 粘贴到通讯工具（Slack、Email 等）
2. 发送给团队成员
...
### 第 5 步：协作者打开分享 URL（接收方）

**为什么**
协作者需要在浏览器中打开 URL 查看内容。
...
1. 从 `window.location.hash` 提取压缩数据
2. 反向执行 Base64 解码 → deflate 解压 → JSON 解析
3. 恢复计划和注释
4. 清除 URL hash（避免刷新时重新加载）
...
**验证只读模式**：
1. 协作者打开分享 URL
2. 检查右上角是否有 "Read-only" 标签
3. 确认 Approve 和 Deny 按钮被禁用

**验证 URL 长度**：
1. 查看 Export Modal 中的 URL 大小
2. 确认不超过 8 KB（如超过，考虑减少注释）
...
源码位置：`apps/hook/server/index.ts:44`
...
**解决方法**：
1. 确保复制完整的 URL（包括 `#` 及后面的所有字符）
2. 不要使用短链接服务（可能会截断 hash）
3. 使用 Export Modal 中的 Copy 按钮，而不是手动复制
...
1. 删除不必要的注释
2. 减少图片附件
3. 考虑使用 Raw Diff 导出并保存为文件
4. 使用代码评审功能（diff 模式的压缩率更高）
...
**技术原理**：
1. **Deflate-raw 压缩**：将 JSON 数据压缩约 60-80%
2. **Base64 编码**：将二进制数据转换为文本
3. **URL 安全字符替换**：`+` → `-`、`/` → `_`、`=` → `''`
4. **Hash 解析**：前端自动解压并恢复内容

**配置选项**：
- `PLANNOTATOR_SHARE=disabled`：禁用分享功能
- 默认启用：分享功能可用

## 下一课预告

> 下一课我们学习 **[Obsidian 集成](../obsidian-integration/)**。
>
> 你会学到：
...
| 恢复注释格式 | [`packages/ui/utils/sharing.ts`](https://github.com/backnotprop/plannotator/blob/main/packages/ui/utils/sharing.ts#L102-L155) | 102-155 |
| 生成分享 URL | [`packages/ui/utils/sharing.ts`](https://github.com/backnotprop/plannotator/blob/main/packages/ui/utils/sharing.ts#L162-L175) | 162-175 |
| 解析 URL hash | [`packages/ui/utils/sharing.ts`](https://github.com/backnotprop/plannotator/blob/main/packages/ui/utils/sharing.ts#L181-L194) | 181-194 |
| URL 大小格式化 | [`packages/ui/utils/sharing.ts`](https://github.com/backnotprop/plannotator/blob/main/packages/ui/utils/sharing.ts#L199-L205) | 199-205 |
| URL 分享 Hook | [`packages/ui/hooks/useSharing.ts`](https://github.com/backnotprop/plannotator/blob/main/packages/ui/hooks/useSharing.ts#L45-L155) | 45-155 |
...
| 分享开关配置（Hook） | [`apps/hook/server/index.ts`](https://github.com/backnotprop/plannotator/blob/main/apps/hook/server/index.ts#L44) | 44 |
| 分享开关配置（OpenCode） | [`apps/opencode-plugin/index.ts`](https://github.com/backnotprop/plannotator/blob/main/apps/opencode-plugin/index.ts#L50) | 50 |

**关键常量**：
- `SHARE_BASE_URL = 'https://share.plannotator.ai'`：分享页面基础域名

**关键函数**：
- `compress(payload: SharePayload): Promise<string>`：压缩 payload 为 base64url 字符串
- `decompress(b64: string): Promise<SharePayload>`：解压 base64url 字符串为 payload
...
- `generateShareUrl(markdown, annotations, attachments): Promise<string>`：生成完整的分享 URL
- `parseShareHash(): Promise<SharePayload | null>`：解析当前 URL 的 hash

---
title: "配置: DCP 多层级设置 | opencode-dcp"
sidebarTitle: "按需定制 DCP"
subtitle: "配置: DCP 多层级设置"
description: "学习 opencode-dcp 的多层级配置系统。掌握全局、环境和项目级配置的优先级规则，修剪策略配置、保护机制设置和通知级别调整。"
tags:
  - "配置"
  - "DCP"
  - "插件设置"
...
每级配置都会覆盖上一级的同名配置项，所以项目配置的优先级最高。
...
确保已完成 [安装与快速开始](../getting-started/)，DCP 插件已成功安装并在 OpenCode 中运行。
...
- `.json`：标准 JSON 格式，不能包含注释
- `.jsonc`：支持 `//` 注释的 JSON 格式（推荐使用）
...
**原因**：
1. 工具名拼写错误
2. 添加到了错误的 `protectedTools` 数组（如 `tools.settings.protectedTools` vs `strategies.deduplication.protectedTools`）
3. 工具调用在回合保护期内（如果启用了回合保护）
...
> 下一课我们学习 **[自动修剪策略详解](../../platforms/auto-pruning/)**。
>
> 你会学到：
> - 去重策略如何检测重复工具调用
> - 覆盖写入策略的工作原理
...
| 配置优先级  | [`lib/config.ts`](https://github.com/Opencode-DCP/opencode-dynamic-context-pruning/blob/main/lib/config.ts#L669-L797) | 669-797   |
| 配置验证    | [`lib/config.ts`](https://github.com/Opencode-DCP/opencode-dynamic-context-pruning/blob/main/lib/config.ts#L147-L375) | 147-375   |
| 配置文件路径 | [`lib/config.ts`](https://github.com/Opencode-DCP/opencode-dynamic-context-pruning/blob/main/lib/config.ts#L484-L526) | 484-526   |
...
| 合并策略配置 | [`lib/config.ts`](https://github.com/Opencode-DCP/opencode-dynamic-context-pruning/blob/main/lib/config.ts#L565-L595) | 565-595   |
| 合并工具配置 | [`lib/config.ts`](https://github.com/Opencode-DCP/opencode-dynamic-context-pruning/blob/main/lib/config.ts#L597-L622) | 597-622   |

**关键常量**：
- `DEFAULT_PROTECTED_TOOLS`：默认受保护的工具名列表（`lib/config.ts:68-79`）

**关键函数**：
- `getConfig()`：加载并合并所有层级的配置（`lib/config.ts:669-797`）
- `getInvalidConfigKeys()`：验证配置文件中的无效键（`lib/config.ts:135-138`）
- `validateConfigTypes()`：验证配置值的类型（`lib/config.ts:147-375`）
- `getConfigPaths()`：获取所有配置文件的路径（`lib/config.ts:484-526`）

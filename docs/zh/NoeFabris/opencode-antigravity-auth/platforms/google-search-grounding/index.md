---
title: "Google Search: 让 Gemini 搜索网络 | Antigravity"
sidebarTitle: "让 Gemini 上网查资料"
subtitle: "Google Search Grounding：让 Gemini 搜索网络信息"
description: "学习为 Gemini 启用 Google Search Grounding，让模型搜索实时网络信息。掌握配置方式和阈值调节技巧，平衡准确性与响应速度。"
tags:
  - "gemini"
  - "google-search"
  - "grounding"
...
::: info 什么是 Google Search Grounding？
...
Google Search Grounding 的核心是让模型**在需要时**自动搜索，而不是每次都搜索。插件通过注入 `googleSearchRetrieval` 工具，让 Gemini 能够调用 Google 搜索 API。
...
在开始前，请确认：
 ...
 - [ ] 已完成 [快速安装](../../start/quick-install/)
 - [ ] 已添加至少一个 Google 账户
 - [ ] 已成功发起首次请求（参考 [首次请求](../../start/first-request/)）
...
- [ ] `grounding_threshold` 在 `0.0` 到 `1.0` 之间
...
**解决方法**：
- 降低 `grounding_threshold` 到 `0.2` 或更低
...
**解决方法**：
- 提高 `grounding_threshold` 到 `0.5` 或更高
...
- 模型会在回答中引用来源，标记为 `[citation:1]` 等

## 下一课预告

> 下一课我们学习 **[双配额系统](../dual-quota-system/)**。
>
> 你会学到：
...
| Google Search 配置加载 | [`src/plugin/config/loader.ts`](https://github.com/NoeFabris/opencode-antigravity-auth/blob/main/src/plugin/config/loader.ts) | 173-184 |
| Google Search 配置应用 | [`src/plugin.ts`](https://github.com/NoeFabris/opencode-antigravity-auth/blob/main/src/plugin.ts) | 1194-1196 |

**关键配置项**：

- `web_search.default_mode`: `"auto"` 或 `"off"`，默认 `"off"`
...
1. 插件从配置文件读取 `web_search.default_mode` 和 `web_search.grounding_threshold`
2. 当 `mode === "auto"` 时，在请求的 `tools` 数组中注入 `googleSearchRetrieval` 工具：
   ```json
   {

---
title: "429排查: 账号轮换与错误处理 | Antigravity-Manager"
sidebarTitle: "429错误：自动轮换账号"
subtitle: "429排查: 账号轮换与错误处理"
description: "学习 Antigravity Tools 的 429 错误排查方法。掌握 QUOTA_EXHAUSTED、RATE_LIMIT_EXCEEDED 等错误的识别与处理，使用 Monitor 和 X-Account-Email 快速定位问题。"
tags:
  - "FAQ"
  - "错误排查"
  - "账号调度"
...
- **等待时间**：`RetryInfo.retryDelay` 或 `details[0].metadata.quotaResetDelay`（如果存在）
...
**你应该看到**：
- 如果响应体里能找到等待时间（例如 `RetryInfo.retryDelay` 或 `quotaResetDelay`），代理通常会先等待一小段时间再重试。
...
3. 重试时强制轮换账号（`attempt > 0` 时 `force_rotate=true`），尝试用下一个可用账号发起上游请求
...
> 下一课我们学习 **[404/路径不兼容：Base URL、/v1 前缀与"叠路径"客户端](../404-base-url/)**。
>
> 你会学到：
> - 最常见的接入错误（404）是如何产生的
...
| 限流原因解析与默认冷却策略 | [`src-tauri/src/proxy/rate_limit.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/rate_limit.rs#L5-L258) | 5-258 |
| MAX_RETRY_ATTEMPTS 常量定义（OpenAI handler） | [`src-tauri/src/proxy/handlers/openai.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/handlers/openai.rs#L14) | 14 |
| 重试次数计算（max_attempts = min(MAX_RETRY_ATTEMPTS, pool_size)） | [`src-tauri/src/proxy/handlers/openai.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/handlers/openai.rs#L830) | 830 |
| OpenAI handler：429/5xx 时标记限流并重试/轮换 | [`src-tauri/src/proxy/handlers/openai.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/handlers/openai.rs#L349-L389) | 349-389 |
| 账号排序优先级（ULTRA > PRO > FREE + remaining_quota） | [`src-tauri/src/proxy/token_manager.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/token_manager.rs#L504-L538) | 504-538 |
| 60s 窗口复用 + 避开限流/配额保护 | [`src-tauri/src/proxy/token_manager.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/token_manager.rs#L624-L739) | 624-739 |
| 限流记录入口（mark_rate_limited） | [`src-tauri/src/proxy/token_manager.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/token_manager.rs#L1089-L1113) | 1089-1113 |
| 429 精确锁定/实时配额刷新/降级策略（mark_rate_limited_async） | [`src-tauri/src/proxy/token_manager.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/token_manager.rs#L1258-L1417) | 1258-1417 |

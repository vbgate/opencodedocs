---
title: "コマンド実行ツールと承認の完全ガイド：セキュリティ仕組み、設定、トラブルシューティング | Clawdbot チュートリアル"
sidebarTitle: "AIに安全にコマンドを実行させる"
subtitle: "コマンド実行ツールと承認"
description: "Clawdbotのexecツールを使用してシェルコマンドを実行する方法、3つの実行モード（sandbox/gateway/node）、セキュリティ承認仕組み、許可リストの設定、承認フローを学習します。このチュートリアルには実践的な設定例、CLIコマンド、トラブルシューティングが含まれており、AIアシスタントの機能を安全に拡張するのに役立ちます。"
tags:
  - "advanced"
  - "tools"
  - "exec"
...
::: warning 前提条件
このチュートリアルは、[Gatewayの起動](../../start/gateway-startup/)が完了しており、Gatewayデーモンが実行中であることを前提としています。
:::
...
1. **ツールポリシー（Tool Policy）**：`tools.policy`で`exec`ツールの許可を制御
...
**重要な違い**：
- sandboxモードはデフォルトで**承認が不要**（ただしSandboxの制限を受ける可能性あり）
- gatewayとnodeモードはデフォルトで**承認が必要**（`full`に設定しない限り）
...
**なぜ**
execツールのパラメータを理解することは、セキュリティ設定の基礎です。
...
**なぜ**
設定ファイルでグローバルデフォルト値を設定することで、exec呼び出しごとにパラメータを指定する必要がなくなります。
...
### ステップ3：`/exec`セッションオーバーライドを使用する

**なぜ**
セッションオーバーライドにより、設定ファイルを編集せずに一時的に実行パラメータを調整できます。
...
**なぜ**
allowlistはgateway/nodeモードの中核となるセキュリティ仕組みであり、特定のコマンドのみを許可します。
...
1. Control UIを開く
2. **Nodes**タブに移動
3. **Exec approvals**カードを見つける
4. ターゲット（GatewayまたはNode）を選択
5. Agentを選択（例：`main`）
6. **Add pattern**をクリックしてコマンドパターンを追加
7. **Save**をクリックして保存
...
`~/.clawdbot/exec-approvals.json`を編集：
...
::: warning 重要なルール
- **解決されたバイナリパスのみマッチ**、basenameマッチ（`rg`など）はサポートされません
...
**なぜ**
セキュアなbinsはstdin操作のみをサポートする安全なバイナリのセットであり、allowlistモードで明示的なallowlistなしで使用できます。
...
**なぜ**
UIが利用できない場合、任意のチャットチャネル（WhatsApp、Telegram、Slackなど）経由でexecリクエストを承認できます。
...
| `Unsupported shell token` | allowlistモードで`>`または`&&`を使用 | コマンドを分割または`security=full`を使用 |
| `Node not found` | nodeモードでノードがペアリングされていない | まずノードペアリングを完了 |

### シェル連結とリダイレクト

::: danger 警告
`security=allowlist`モードでは、以下のシェル機能は**サポートされません**：
- パイプ：`|`（ただし`||`はサポート）
- リダイレクト：`>`、`<`、`>>`
- コマンド置換：`$()`、`` ` ` ``
- バックグラウンド：`&`、`;`
...
> 次のレッスンでは、**[Web検索とスクレイピングツール](../tools-web/)**を学習します
>
> 学ぶ内容：
> - `web_search`ツールを使用してWeb検索を行う方法
> - `web_fetch`ツールを使用してWebページコンテンツをスクレイピングする方法
...
- `ExecAllowlistEntry`: allowlistエントリタイプ（`pattern`、`lastUsedAt`などを含む）

---
title: "隠蔽モード: 幅計算の原理 | opencode-md-table-formatter"
sidebarTitle: "幅計算の3ステップ"
subtitle: "隠蔽モードの原理：なぜ幅計算が重要なのか"
description: "OpenCode 隠蔽モードの動作原理を学習します。プラグインの表示幅計算方法を習得し、Markdown 記号の除去、コードブロックの保護、Bun.stringWidth の使用方法を理解します。"
tags:
  - "concealment-mode"
  - "表示幅計算"
  - "Markdown 記号の除去"
...
このプラグインのコアアイデアは：**ユーザーが実際に見る幅を計算し、ソースコードの文字数を計算しないこと**です。
...
- [ ] `Bun.stringWidth` は `text.length` より何が優れていますか？（答：日本語、Emoji、ゼロ幅文字の表示幅を正しく計算できる）
...
> 次のレッスンでは **[テーブル仕様](../table-spec/)** を学習します。
>
> 学習内容：
...
| 機能 | ファイルパス | 行番号 |
|--- | --- | ---|
| 表示幅計算のエントリーポイント | [`index.ts`](https://github.com/franlol/opencode-md-table-formatter/blob/main/index.ts#L151-L159) | 151-159 |
| コードブロックの保護 | [`index.ts`](https://github.com/franlol/opencode-md-table-formatter/blob/main/index.ts#L168-L173) | 168-173 |
| Markdown 記号の除去 | [`index.ts`](https://github.com/franlol/opencode-md-table-formatter/blob/main/index.ts#L175-L188) | 175-188 |
| コードブロックの復元 | [`index.ts`](https://github.com/franlol/opencode-md-table-formatter/blob/main/index.ts#L190-L193) | 190-193 |
| Bun.stringWidth の呼び出し | [`index.ts`](https://github.com/franlol/opencode-md-table-formatter/blob/main/index.ts#L195) | 195 |

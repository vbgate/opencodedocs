...
const { localeIndex, theme, page } = vitePressData
...
const rawResults: Ref<(SearchResult & Result)[]> = shallowRef([])
...
debouncedWatch(
  ...
  async ([index, filterTextValue, showDetailedListValue], old, onCleanup) => {
    ...
    const mods = showDetailedListValue
      ? await Promise.all(rawResults.value.map((r) => fetchExcerpt(r.id)))
      : []
    if (canceled) return
    ...
    for (const { id, mod } of mods) {
      ...
      if (comp?.render || comp?.setup) {
        ...
        headings.forEach((el: any) => {
          ...
          if (!anchor) return
          let html = ''
          let sibling: any = el.nextElementSibling
    ...
    rawResults.value = rawResults.value.map((r) => {
...
const searchScope = useLocalStorage('vitepress:search-scope', 'all' as 'all' | 'project')
...
const showScopeToggle = computed(() => {
  return !!projectPrefix.value
})

const results = computed(() => {
  if (searchScope.value === 'project' && projectPrefix.value) {
    return rawResults.value.filter(r => r.id.startsWith(projectPrefix.value!))
  }
  return rawResults.value
})

watch(projectPrefix, (newPrefix) => {
  if (!newPrefix) {
...
const scopeLabels = computed(() => {
  const labels = theme.value.search?.options?.scopeLabels as any
  if (!labels) {
    return {
      all: 'All Site',
  ...
  return labels[localeIndex.value] || labels.root || labels
})
  ...
  <Teleport to="body">
    ...
    >
      ...
      <div class="shell">
        ...
        <div v-if="showScopeToggle" class="scope-toggle">
          <button
            type="button"
            class="scope-button"
          ...
          <button
            type="button"
            class="scope-button"
            :class="{ active: searchScope === 'project' }"
            @click="searchScope = 'project'"
          >
            {{ scopeLabels.project }}
          </button>
        </div>
...
.excerpt-gradient-bottom {
  bottom: 0;
  background: linear-gradient(transparent, var(--vp-local-search-bg));
}

.excerpt-gradient-top {
  top: 0;
  background: linear-gradient(var(--vp-local-search-bg), transparent);
}

---
title: "Полное руководство по системе памяти и векторному поиску | Учебник Clawdbot"
sidebarTitle: "Настроить поиск памяти"
subtitle: "Полное руководство по системе памяти и векторному поиску"
description: "Узнайте, как работает система памяти Clawdbot, включая векторную индексацию, полнотекстовый поиск FTS5, гибридный поиск и настройку поставщиков Embedding. Освойте настройку и использование долгосрочной памяти ИИ для повышения точности семантического поиска."
tags:
  - "memory"
  - "vector-search"
  - "embedding"
...
|--- | --- | ---|
| Менеджер памяти | [`src/memory/manager.ts`](https://github.com/moltbot/moltbot/blob/main/src/memory/manager.ts) | 1-200 |
| Гибридный поиск | [`src/memory/hybrid.ts`](https://github.com/moltbot/moltbot/blob/main/src/memory/hybrid.ts) | 1-112 |
| Поставщик Embedding | [`src/memory/embeddings.ts`](https://github.com/moltbot/moltbot/blob/main/src/memory/embeddings.ts) | 1-80 |
| OpenAI embeddings | [`src/memory/embeddings-openai.ts`](https://github.com/moltbot/moltbot/blob/main/src/memory/embeddings-openai.ts) | Весь файл |
| Gemini embeddings | [`src/memory/embeddings-gemini.ts`](https://github.com/moltbot/moltbot/blob/main/src/memory/embeddings-gemini.ts) | Весь файл |
| Локальные embeddings | [`src/memory/node-llama.ts`](https://github.com/moltbot/moltbot/blob/main/src/memory/node-llama.ts) | Весь файл |
| SQLite-vec | [`src/memory/sqlite-vec.ts`](https://github.com/moltbot/moltbot/blob/main/src/memory/sqlite-vec.ts) | Весь файл |
| Пакетная индексация (OpenAI) | [`src/memory/batch-openai.ts`](https://github.com/moltbot/moltbot/blob/main/src/memory/batch-openai.ts) | Весь файл |
| Пакетная индексация (Gemini) | [`src/memory/batch-gemini.ts`](https://github.com/moltbot/moltbot/blob/main/src/memory/batch-gemini.ts) | Весь файл |
| Менеджер поиска | [`src/memory/manager-search.ts`](https://github.com/moltbot/moltbot/blob/main/src/memory/manager-search.ts) | Весь файл |
| Memory Schema | [`src/memory/memory-schema.ts`](https://github.com/moltbot/moltbot/blob/main/src/memory/memory-schema.ts) | Весь файл |

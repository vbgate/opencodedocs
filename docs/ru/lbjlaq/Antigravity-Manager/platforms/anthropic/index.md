---
title: "Anthropic: Совместимый API | Antigravity-Manager"
sidebarTitle: "Пусть Claude Code использует локальный шлюз"
subtitle: "Anthropic: Совместимый API"
description: "Изучите API Anthropic Antigravity-Manager. Настройте Base URL Claude Code, реализуйте диалог с помощью /v1/messages, поймите аутентификацию и перехват warmup."
tags:
  - "anthropic"
  - "claude"
  - "claude-code"
...
Этот урок предполагает, что вы уже реализовали базовый цикл локального прокси (можете получить доступ к `/healthz`, знаете порт прокси и включена ли аутентификация). Если ещё не реализовано, сначала прочитайте **[Запуск локального прокси и подключение первого клиента](/ru/lbjlaq/Antigravity-Manager/start/proxy-and-first-client/)**.
:::
...
**Зачем**
`/v1/messages` — это фиксированный маршрут на стороне прокси, написание Base URL как `.../v1` легко приводит к тому, что клиент снова склеит `/v1/messages`, в итоге получится `.../v1/v1/messages`.
...
**Зачем**
Промежуточное ПО аутентификации прокси берёт key из `Authorization`, `x-api-key`, `x-goog-api-key`; если включена аутентификация, но заголовок не совпадает, будет стабильный 401.

::: info Какие заголовки аутентификации принимает прокси?
...
**Зачем**
Claude Code использует протокол Anthropic Messages; указав его Base URL на локальный шлюз, можно повторно использовать этот контракт `/v1/messages`.
...
**Зачем**
Разные клиенты передадут `model` как есть; сначала получите список моделей, устранение неполадок будет намного быстрее.
...
**Зачем**
Это минимально воспроизводимая цепочка: без Claude Code тоже можно подтвердить, где именно ошибка в «маршрутизация + аутентификация + тело запроса».
...
**Зачем**
Claude Code будет использовать SSE; вы также можете использовать curl для реализации потока SSE, подтверждая, что между ними нет проблем с прокси/буфером.
...
> В следующем уроке мы изучим **[Нативный API Gemini: /v1beta/models и подключение конечных точек Google SDK](/ru/lbjlaq/Antigravity-Manager/platforms/gemini/)**.

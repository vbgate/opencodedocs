---
title: "OpenAI API: Конфигурация подключения | Antigravity-Manager"
sidebarTitle: "Подключение к OpenAI SDK за 5 минут"
subtitle: "OpenAI API: Конфигурация подключения"
description: "Изучите конфигурацию подключения для API, совместимого с OpenAI. Освойте преобразование маршрутов, настройку base_url и устранение ошибок 401/404/429, чтобы быстро использовать Antigravity Tools."
tags:
  - "OpenAI"
  - "API Proxy"
  - "Chat Completions"
...
Многие клиенты/SDK распознают только форму интерфейса OpenAI: фиксированный URL, фиксированные поля JSON, фиксированный формат потока SSE.
Цель Antigravity Tools — не заставить вас изменить клиент, а позволить клиенту «думать, что он вызывает OpenAI», фактически преобразуя запрос во внутренний вызов вышестоящего уровня, а затем преобразуя результат обратно в формат OpenAI.
...
::: info Как принести аутентификацию?
...
**Зачем**
Сначала исключите низкоуровневые проблемы типа «сервис не запущен/порт неправильный/заблокирован брандмауэром».
...
**Зачем**
Этот шаг доказывает, что вся цепочка «OpenAI SDK → локальный шлюз → вышестоящий уровень → преобразование ответа OpenAI» проходит успешно.
...
**Зачем**
Многие клиенты зависят от протокола SSE OpenAI (`Content-Type: text/event-stream`). Этот шаг подтверждает, что потоковая цепочка и формат события доступны.
...
**Зачем**
Некоторые инструменты используют `/v1/responses` или используют поля `instructions`, `input` в теле запроса. Этот проект нормализует такие запросы как `messages` и затем повторно использует тот же набор логики преобразования. (Обработчик: `src-tauri/src/proxy/handlers/openai.rs`)
...
::: tip Подсказка по режиму аутентификации
...
|--- | --- | ---|
| Регистрация маршрута OpenAI (включая /v1/responses) | [`src-tauri/src/proxy/server.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/server.rs#L120-L194) | 120-194 |
| Обработчик Chat Completions (включая обнаружение формата Responses) | [`src-tauri/src/proxy/handlers/openai.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/handlers/openai.rs#L70-L462) | 70-462 |
| Обработчик /v1/completions и /v1/responses (нормализация Codex/Responses + повтор/ротация) | [`src-tauri/src/proxy/handlers/openai.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/handlers/openai.rs#L464-L1080) | 464-1080 |
| Возврат /v1/models (динамический список моделей) | [`src-tauri/src/proxy/handlers/openai.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/handlers/openai.rs#L1082-L1102) | 1082-1102 |
| Структура данных запроса OpenAI (messages/instructions/input/size/quality) | [`src-tauri/src/proxy/mappers/openai/models.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/mappers/openai/models.rs#L7-L38) | 7-38 |
|--- | --- | ---|
|--- | --- | ---|
| Сопоставление моделей и приоритет подстановочных знаков (точный > подстановочный > по умолчанию) | [`src-tauri/src/proxy/common/model_mapping.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/common/model_mapping.rs#L180-L228) | 180-228 |
|--- | --- | ---|

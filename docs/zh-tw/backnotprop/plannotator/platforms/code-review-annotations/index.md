---
title: "程式碼批註: 行級註解 | Plannotator"
subtitle: "程式碼批註: 行級註解"
sidebarTitle: "5 分鐘加批註"
description: "學習 Plannotator 的程式碼註解功能。在 diff 中精確新增行級註解（comment/suggestion/concern）、附加建議程式碼、標記風險點、管理所有註解並匯出回饋。"
tags:
  - "程式碼審查"
  - "註解"
  - "diff"
...
- ✅ 已學會程式碼審查基礎（詳見 [程式碼審查基礎](../code-review-basics/)）
...
**程式碼註解**是 Plannotator 程式碼審查的核心功能，用於在 Git diff 中新增行級回饋。透過點擊行號選取程式碼範圍，你可以精確地針對具體程式碼行新增評論、建議或關注點，註解會顯示在 diff 下方，便於 AI 準確理解你的回饋意圖。
...
**你應該看到**：
- 工具列顯示選取行號（例如 `Line 10` 或 `Lines 10-15`）

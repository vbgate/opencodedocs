---
title: "計畫批註: 四種註解類型 | Plannotator"
sidebarTitle: "新增四種註解類型"
subtitle: "計畫批註: 四種註解類型"
description: "掌握 Plannotator 四種註解類型的使用方法。學習刪除、取代、插入、評論的新增技巧，支援 type-to-comment 快捷輸入和圖片附件，提升計畫審查效率。"
tags:
  - "計畫註解"
  - "註解類型"
  - "刪除"
...
**前置條件**：
- ✅ 已完成 [計畫審查基礎](../plan-review-basics/) 教學
...
選取文字 → 工具列彈出（選單步驟）
                 ...
                 └── 直接輸入字元 → type-to-comment → 自動進入輸入步驟
```
...
# 等待 AI 完成計畫，Plannotator 會自動在瀏覽器中開啟
```
...
**為什麼**
刪除註解用於標記不希望出現在最終計畫中的內容。

**操作**

1. 在計畫中找到你不需要的段落（例如某個不相關的功能描述）
2. 滑鼠選取文字
3. 工具列自動彈出，點擊 **刪除按鈕（🗑️）**
...
::: tip 刪除註解的特點
刪除註解**不需要輸入任何內容**。點擊刪除按鈕後，註解立即建立完成。
:::

### 第 3 步：使用 type-to-comment 新增評論

**為什麼**
評論是最常用的註解類型，type-to-comment 能讓你少點一次按鈕。

**操作**

1. 在計畫中選取文字（例如某個函式名稱或描述）
2. **不要點擊任何按鈕，直接開始打字**
3. 輸入你的評論內容（例如：「這個函式名稱不夠清晰」）
4. 按 `Enter` 鍵儲存，或點擊 **Save** 按鈕

**你應該看到**：
- 工具列自動切換到輸入框模式
- 你輸入的第一個字元已經在輸入框中
- 游標自動定位到輸入框末尾
- 按 `Enter` 後，選取的文字顯示為評論樣式（通常是黃色背景）

::: tip type-to-comment 的快捷鍵
- `Enter`：儲存註解（如果輸入框有內容）
- `Shift + Enter`：換行（輸入多行評論時使用）
- `Escape`：取消輸入，返回選單步驟
:::
...
**為什麼**
取代註解用於用新內容取代選取內容，AI 會根據你的註解修改計畫。

**操作**

1. 在計畫中選取文字（例如「使用 JWT token 進行認證」）
2. 使用 type-to-comment 或點擊評論按鈕
3. 在輸入框中輸入新的內容（例如：「使用 session cookie 進行認證」）
4. 按 `Enter` 儲存
...
**為什麼**
插入註解用於在選取內容後補充說明或程式碼片段。

**操作**

1. 在計畫中選取文字（例如函式簽章的末尾）
2. 使用 type-to-comment 或點擊評論按鈕
3. 在輸入框中輸入要插入的內容（例如：「，需要處理認證失敗的情況」）
4. 按 `Enter` 儲存
...
**為什麼**
有時候文字描述不夠直觀，需要附加參考圖或截圖。

**操作**

1. 選取任意文字，進入輸入步驟（點擊評論按鈕或 type-to-comment）
2. 在工具列輸入框旁邊，點擊 **附件按鈕（📎）**
3. 選擇要上傳的圖片（支援 PNG、JPEG、WebP 格式）
4. 可以繼續輸入評論內容
5. 按 `Enter` 儲存
...
**為什麼**
當你對整個計畫有回饋（不是針對具體某段文字）時，使用全域評論。

**操作**

1. 在頁面底部找到輸入框（標籤可能是「Add a general comment about the plan...」）
2. 輸入你的評論內容
3. 按 `Enter` 儲存或點擊傳送按鈕
...
**可能原因**：
1. 沒有選取文字
2. 先點擊了某個按鈕，工具列已進入輸入步驟
3. 輸入了特殊鍵（`Ctrl`、`Alt`、`Escape` 等）

**正確做法**：
1. 先選取文字，確保工具列顯示在選單步驟（有 Delete、Comment 按鈕）
2. 直接輸入普通字元（字母、數字、標點）
3. 不要按功能鍵
...
- 刪除：`## Remove this` + 原文字
- 插入：`## Add this` + 新文字
- 取代：`## Change this` + From/To 對比
- 評論：`## Feedback on: "..."` + 評論內容

## 下一課預告

> 下一課我們學習 **[新增圖像標註](../plan-review-images/)**。
>
> 你會學到：
> - 如何在計畫審查中附加圖像
> - 使用畫筆、箭頭、圓形工具進行標註
...
**關鍵常數**：
- `AnnotationType.DELETION = 'DELETION'`：刪除註解類型
- `AnnotationType.INSERTION = 'INSERTION'`：插入註解類型
- `AnnotationType.REPLACEMENT = 'REPLACEMENT'`：取代註解類型
- `AnnotationType.COMMENT = 'COMMENT'`：評論註解類型
- `AnnotationType.GLOBAL_COMMENT = 'GLOBAL_COMMENT'`：全域評論類型

**關鍵函式**：
- `exportDiff(blocks, annotations)`：將註解匯出為 Markdown 格式，包含 From/To 對比
- `parseMarkdownToBlocks(markdown)`：將 Markdown 解析為線性 Block 陣列
- `createAnnotationFromSource()`：從文字選取建立 Annotation 物件

---
title: "429排查: 帳號輪換與錯誤處理 | Antigravity-Manager"
sidebarTitle: "429錯誤：自動輪換帳號"
subtitle: "429排查: 帳號輪換與錯誤處理"
description: "學習 Antigravity Tools 的 429 錯誤排查方法。掌握 QUOTA_EXHAUSTED、RATE_LIMIT_EXCEEDED 等錯誤的識別與處理，使用 Monitor 和 X-Account-Email 快速定位問題。"
tags:
  - "FAQ"
  - "錯誤排查"
  - "帳號調度"
...
- **原因**：`error.details[0].reason`（例如 `RATE_LIMIT_EXCEEDED`、`QUOTA_EXHAUSTED`）或 `error.message`
- **等待時間**：`RetryInfo.retryDelay` 或 `details[0].metadata.quotaResetDelay`（如果存在）
...
**你應該看到**：
- 如果響應體裡能找到等待時間（例如 `RetryInfo.retryDelay` 或 `quotaResetDelay`），代理通常會先等待一小段時間再重試。
...
| **Balance** | 啟用黏性會話與 60s 視窗複用 | **立即切換**到備選帳號，兼顧成功率和效能 | 通用場景，預設 |
...
3. 重試時強制輪換帳號（`attempt > 0` 時 `force_rotate=true`），嘗試用下一個可用帳號發起上游請求
...
> 下一課我們學習 **[404/路徑不相容：Base URL、/v1 前綴與「疊路徑」客戶端](../404-base-url/)**。
>
> 你會學到：
> - 最常見的接入錯誤（404）是如何產生的
...
| 功能 | 檔案路徑 | 行號 |
| :--- | :--- | :--- |
| 429 重試延遲解析（RetryInfo / quotaResetDelay） | [`src-tauri/src/proxy/upstream/retry.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/upstream/retry.rs#L38-L67) | 38-67 |
...
| 調度模式列舉（CacheFirst/Balance/PerformanceFirst） | [`src-tauri/src/proxy/sticky_config.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/sticky_config.rs#L3-L12) | 3-12 |
| 限流原因解析與預設冷卻策略 | [`src-tauri/src/proxy/rate_limit.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/rate_limit.rs#L5-L258) | 5-258 |
| MAX_RETRY_ATTEMPTS 常數定義（OpenAI handler） | [`src-tauri/src/proxy/handlers/openai.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/handlers/openai.rs#L14) | 14 |
| 重試次數計算（max_attempts = min(MAX_RETRY_ATTEMPTS, pool_size)） | [`src-tauri/src/proxy/handlers/openai.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/handlers/openai.rs#L830) | 830 |
| OpenAI handler：429/5xx 時標記限流並重試/輪換 | [`src-tauri/src/proxy/handlers/openai.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/handlers/openai.rs#L349-L389) | 349-389 |
| 帳號排序優先級（ULTRA > PRO > FREE + remaining_quota） | [`src-tauri/src/proxy/token_manager.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/token_manager.rs#L504-L538) | 504-538 |
| 60s 視窗複用 + 避開限流/配額保護 | [`src-tauri/src/proxy/token_manager.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/token_manager.rs#L624-L739) | 624-739 |
| 限流記錄入口（mark_rate_limited） | [`src-tauri/src/proxy/token_manager.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/token_manager.rs#L1089-L1113) | 1089-1113 |
| 429 精確鎖定/即時配額刷新/降級策略（mark_rate_limited_async） | [`src-tauri/src/proxy/token_manager.rs`](https://github.com/lbjlaq/Antigravity-Manager/blob/main/src-tauri/src/proxy/token_manager.rs#L1258-L1417) | 1258-1417 |

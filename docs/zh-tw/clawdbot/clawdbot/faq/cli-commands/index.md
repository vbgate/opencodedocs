---
title: "CLI 命令參考：完整命令列表與用法 | Clawdbot 教程"
sidebarTitle: "命令查不到了"
subtitle: "CLI 命令參考：完整的命令列表和用法說明"
description: "學習 Clawdbot 所有 CLI 命令的完整參考，包括 gateway、agent、models、channels、nodes、skills 等 24 個命令組的詳細用法、選項說明和實戰範例。快速查找任何 CLI 命令的用法和參數，透過範例快速上手複雜命令，有效管理 Gateway、Agent、渠道、定時任務、設備節點、技能包和瀏覽器自動化。"
tags:
  - "CLI"
  - "命令參考"
  - "FAQ"
...
**选项**：
- `--url <url>` - Gateway WebSocket URL（預設使用配置/远程/本地）
- `--token <token>` - Gateway token（如需要）
- `--password <password>` - Gateway 密码（密码认证）
- `--timeout <ms>` - 超时时间（毫秒），預設 10000
- `--no-probe` - 跳过 RPC 探测
- `--deep` - 扫描系统级服务
- `--json` - 输出 JSON 格式
...
- `--runtime <runtime>` - 守护进程运行时（node|bun），預設 node
...
- `-t, --to <number>` - 用于派生會話密钥的接收者号码（E.164 格式）
- `--session-id <id>` - 使用显式會話 ID
- `--agent <id>` - Agent ID（覆盖路由绑定）
- `--thinking <level>` - 思考级别：off | minimal | low | medium | high
- `--verbose <on|off>` - 为會話持久化 Agent 详细级别
- `--channel <channel>` - 投递渠道
- `--reply-to <target>` - 投递目标覆盖（与會話路由分离）
- `--reply-channel <channel>` - 投递渠道覆盖（与會話路由分离）
- `--reply-account <id>` - 投递账户 ID 覆盖
- `--local` - 在本地运行嵌入式 Agent（需要在 shell 中配置模型提供商 API 密钥）
- `--deliver` - 将 Agent 的回复发送回所选渠道
- `--json` - 输出结果为 JSON
- `--timeout <seconds>` - 覆盖 Agent 命令超时（秒），預設 600 或配置值
...
> 下一课我们学习 **[性能優化](../performance-optimization/)**。
>
> 你会学到：
> - 如何優化 Gateway 性能
> - 减少延遲的方法

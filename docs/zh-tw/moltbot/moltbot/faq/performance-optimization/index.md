---
title: "性能優化：提升 Gateway 響應速度與記憶體管理 | Clawdbot 教程"
sidebarTitle: "響應慢怎麼辦"
subtitle: "性能優化：提升 Gateway 響應速度與記憶體管理"
description: "學習如何優化 Clawdbot 性能，包括提示詞快取（Prompt Caching）、會話修剪（Session Pruning）、併發控制、記憶體管理和日誌調優，降低延遲、減少成本並提升響應速度。"
tags:
  - "性能優化"
  - "會話管理"
  - "併發控制"
...
---
title: "性能優化：提升 Gateway 响应速度与記憶體管理 | Clawdbot 教程"
sidebarTitle: "响应慢怎么办"
subtitle: "性能優化：提升 Gateway 响应速度与記憶體管理"
description: "学习如何優化 Clawdbot 性能，包括提示詞快取（Prompt Caching）、會話修剪（Session Pruning）、併發控制、記憶體管理和日志调优，降低延遲、减少成本并提升响应速度。"
tags:
  - "性能優化"
  - "會話管理"
  - "併發控制"
...
# 性能優化：提升 Gateway 响应速度与記憶體管理

**性能優化**是让 Clawdbot 保持快速响应、降低 API 成本和稳定运行的关键。本教程教你如何優化 Gateway 性能，包括提示詞快取、會話修剪、併發控制、記憶體管理和日志调优。

## 学完你能做什么

- 配置**提示詞快取**（Prompt Caching）以大幅减少 Anthropic API 调用成本
- 启用**會話修剪**（Session Pruning）防止上下文无限膨胀
- 调整**併發控制**（Concurrency）避免资源争用
- 優化**日志级别**和 **OpenTelemetry 采样率**减少 IO 开销
- 配置**記憶體刷新**（Memory Flush）和 **Compaction**保持对话流畅
- 使用**本地模型**时優化硬件配置
...
- **响应越来越慢**：早期对话很快，越往后越慢
- **API 成本飙升**：同样的对话重复计费
- **記憶體占用高**：Gateway 进程占用几百 MB 甚至 GB
- **队列积压**：消息处理延遲明显
- **上下文溢出**：频繁遇到"上下文已满"错误
:::
...
3. **无併發控制**：多个會話同时竞争 CPU/記憶體/网络
4. **日志过于详细**：大量 debug 日志消耗磁盘 I/O

::: tip 一句话总结
> 優化核心就是：**减少重复计算**、**控制资源使用**、**利用快取**。
:::

## 核心思路

Clawdbot 的性能優化围绕三个层面：

1. **Prompt 层面**：减少每次 API 调用的 token 数量
2. **Session 层面**：控制历史对话在記憶體中的增长
3. **Gateway 层面**：管理併發、日志、诊断开销
...
## 優化 1：启用提示詞快取（Prompt Caching）

### 什么是提示詞快取

Anthropic API 支持**提示詞快取**（Prompt Caching）：相同的对话历史可以快取，避免重复计费。

::: info 适用条件
- 仅适用于 **Anthropic 模型**（Claude）
- 需要 **會話粘性**（Session Stickiness）：即同一个會話保持使用同一个认证配置
- 預設**已启用**（通过 `auth.json` 自动管理）
:::
...
| 场景               | 无快取                          | 有快取                          |
|--- | --- | ---|
| 每轮都发送全历史 | 200,000 tokens × 10 轮 = 2M tokens | 第 1 轮 200K，后续 9 轮 ~100K（快取命中） |
| 成本（Anthropic） | ≈$30                          | ≈$3.5（省 88%）           |
| 延遲             | 每轮都需全历史                 | 快取命中只需发送增量内容     |

### 預設配置

**好消息**：如果你使用 **OAuth** 或 **setup-token** 方式配置 Anthropic，Clawdbot 預設已启用快取。
...
::: warning 注意事项
- API Key 配置也支持快取，但需要手动设置 `cacheControlTtl`
- 快取只在 **Anthropic API** 上有效，其他提供商不支持
:::

---

## 優化 2：启用會話修剪（Session Pruning）
...
```json5
{
  agent: {
    contextPruning: {
      mode: "cache-ttl",     // 使用 Anthropic 的快取 TTL 作为修剪时机
      ttl: "5m",              // 5 分钟后开始修剪
...
| API 调用延遲     | 随时间增长明显                   | 相对稳定                        |

---

## 優化 3：配置压缩与記憶體刷新（Compaction + Memory Flush）
...
### 什么是記憶體刷新（Memory Flush）

在压缩之前，Clawdbot 可以执行一个**静默的記憶體刷新**（Memory Flush）：
...
```json5
{
  agent: {
    compaction: {
      autoCompact: true,           // 自动压缩（預設启用）
      memoryFlush: {
        enabled: true,              // 启用記憶體刷新
...
## 優化 4：调整併發控制（Concurrency）

### 队列架构

Clawdbot 使用**车道队列**（Lane Queue）管理併發：

- **主车道**（`main`）：所有會話共享
- **子车道**（`cron`、`subagent`）：独立併發池
- **會話车道**：每个會話有独立队列，保证顺序执行
...
{
  agent: {
    maxConcurrent: 4,           // 全局最大併發（預設：4）
    maxConcurrentPerSession: 1  // 每會話最大併發（預設：1）
  }
}
```
...
| 高频消息（群聊）     | 4-8      | 防止队列积压，保持低延遲       |
| 资源受限（低配设备） | 1-2      | 避免記憶體溢出，系统卡顿         |
| 大模型（本地 200B）  | 1-2      | GPU 記憶體有限，单任务更稳定         |
...
## 優化 5：優化日志与诊断（Logging + Diagnostics）
...
`web_search` 和 `web_fetch` 工具会快取结果，减少重复请求。
...
```json5
{
  tools: {
    web: {
      search: {
        cacheTtlMinutes: 15    // 搜索结果快取 15 分钟（預設）
      },
      fetch: {
        cacheTtlMinutes: 15    // 抓取内容快取 15 分钟（預設）
      }
    }
  }
}
...
Browser 工具会启动独立的 Chrome/Chromium 进程，預設有 2 个併發实例。
...
| 模型大小   | GPU 記憶體（推荐） | 系统記憶體（推荐） | CPU（最低） |
|--- | --- | --- | ---|
...
::: warning 常见问题
- **記憶體不足（OOM）：日志显示 `Out of memory`，需要增加記憶體
...
|--- | --- | ---|
| 首字响应延遲     | < 2 秒     | > 5 秒      |
| 會話上下文大小     | < 100K tokens | > 150K tokens |
| 队列等待时间       | < 2 秒     | > 10 秒      |
| CPU 使用率（持续） | < 50%      | > 80%       |
| 記憶體使用率（持续） | < 70%      | > 90%       |
...
**问题**：在低配设备上设置 `maxConcurrent: 10` 会导致記憶體溢出。
...
本课我们学习了 Clawdbot 的**完整性能優化策略**：

### 核心優化点

1. ✅ **提示詞快取**：减少 Anthropic API 成本 70%+
2. ✅ **會話修剪**：控制上下文增长，避免溢出
3. ✅ **压缩与刷新**：自动总结历史，保存持久记忆
4. ✅ **併發控制**：平衡资源使用，防止队列积压
5. ✅ **日志優化**：调整级别，减少 I/O 开销
6. ✅ **OpenTelemetry 采样**：监控而不影响性能
7. ✅ **工具快取**：减少重复网络请求
8. ✅ **硬件配置**：本地模型的記憶體和 CPU 優化
...
| 指标           | 優化前   | 優化后   | 改善     |
|--- | --- | --- | ---|
| 平均响应延遲     | 3-5 秒  | 1-2 秒  | -50%     |
| 单次 API 成本   | $0.50   | $0.15    | -70%     |
| 記憶體占用（稳定）   | 1.2 GB   | 400 MB    | -67%     |
| 會話稳定性       | 85%      | 98%      | +13%     |
...
- `DEFAULT_CACHE_TTL_MINUTES = 15`：Web 工具預設快取时间
- `DEFAULT_OTEL_SAMPLE_RATE = 0.2`：OpenTelemetry 預設采样率

**关键函数**：
- `pruneContext()`：修剪工具结果的函数
- `compactHistory()`：压缩历史对话的函数
- `flushMemory()`：执行記憶體刷新的函数
- `drainLane()`：队列处理函数

---
title: "快速開始: 安裝 DCP 外掛 | OpenCode DCP"
sidebarTitle: "快速開始"
subtitle: "快速開始: 安裝 DCP 外掛"
description: "學習在 5 分鐘內完成 OpenCode DCP 外掛安裝。配置外掛、驗證安裝、查看修剪效果，透過自動去重和 AI 驅動修剪降低 Token 消耗。"
tags:
  - "安裝"
  - "快速開始"
  - "DCP"
...
3. **AI 驅動**：AI 可以主動呼叫 `discard` 和 `extract` 工具優化上下文
...
**為什麼**
需要在 OpenCode 配置中宣告 DCP 外掛，這樣 OpenCode 啟動時會自動載入。
...
如果你需要自訂配置，可以手動新增這些欄位。詳細配置說明見[配置全解](../configuration/)。
:::
...
3. 在 `opencode.jsonc` 中檢查外掛名稱是否正確：`@tarquinen/opencode-dcp@latest`
...
| Token 計算  | [`lib/commands/context.ts`](https://github.com/Opencode-DCP/opencode-dynamic-context-pruning/blob/main/lib/commands/context.ts) | 68-174  |
